...
function mergeWaterData() {
  ...
  console.log(`Found ${files.length} JSON files to process:`, files.map(f => path.basename(f)));
  ...
  files.forEach(filePath => {
    ...
    try {
      ...
      if (!Array.isArray(data)) {
        console.warn(`Warning: ${fileName} does not contain an array, skipping.`);
        return;
      }
      ...
      data.forEach(record => {
        if (!record.lake || !record.date) {
          console.warn(`Warning: Record missing lake or date in ${fileName}, skipping:`, record);
          return;
        }
        ...
        if (mergedData.has(mergeKey)) {
          console.log(`Duplicate key found: ${mergeKey}, keeping first occurrence`);
